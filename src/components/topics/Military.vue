...
export default {
  ...
  mounted() {
    axios
      .get(
        "http://api.tianapi.com/military/?key=cf3c751948de97ae55bc930010cef3c2&num=10"
      )
      .then(response => {
  ...
  methods: {
    infiniteHandler($state) {
      setTimeout(() => {
        axios
          .get(
            "http://api.tianapi.com/military/?key=cf3c751948de97ae55bc930010cef3c2&num=10&page=" +
              ++this.page
          )
          .then(response => {
            // 把查询到的数据放到 items里
